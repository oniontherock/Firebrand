...
DoorGenerator::WallSectionCell::WallSectionCell(RoomType _first, RoomType _second, WallSectionId _sectionId) {
	first = _first;
	second = _second;
	sectionId = _sectionId;
}

DoorGenerator::WallSectionGrid2D DoorGenerator::wallSectionsGet(const WallGrid2D wallGrid, RoomTypeGrid& roomTypeGrid, const sf::Vector2u structureSize) {
	...
	for (uint16_t cellX = 1; cellX < structureSize.x - 1; cellX++) {
		for (uint16_t cellY = 1; cellY < structureSize.y - 1; cellY++) {
			...
			bool isValidSection = sectionIsHorizontal != sectionIsVertical;
			...
			std::pair<sf::Vector2i, sf::Vector2i> sideOffsets(Vector2iMath::rotate(frontOffsets.second, -Mathf::PI / 2.f), Vector2iMath::rotate(frontOffsets.first, -Mathf::PI / 2.f));
			...
			while (!cellsToEvaluate.empty()) {
				...
				if (!cellsEvaluated.contains(cellFront)) cellsToEvaluate.push(cellFront);
				// if cellBack has yet to be evaluated, queue it for evaluation
				if (!cellsEvaluated.contains(cellBack)) cellsToEvaluate.push(cellBack);
			}
