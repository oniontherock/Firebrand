...
std::pair<StructureGenerator::WallType, float> StructureGenerator::wallDataGetFromSurroundings(SurroundingWallsVector wallStates) {
	WallType wallType;
...
const char* StructureGenerator::cellTypeGetFromWallType(WallType wallType) {
	switch (wallType) {
	case None:
		return "Null";
	case Single:
...
StructureGrid StructureGenerator::structureGenerate(StructureType structureType, sf::Vector2f structurePosition, float structureRotation, sf::Vector2i structureSize) {
	...
	for (uint16_t x = 0; x < structureSize.x; x++) {
		for (uint16_t y = 0; y < structureSize.y; y++) {
			...
			std::pair<WallType, float> wallData = wallDataGetFromSurroundings(wallStates);

			StructureCell cell = StructureCell(cellTypeGetFromWallType(wallData.first), wallData.second);
